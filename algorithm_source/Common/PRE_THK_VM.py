...
class PRE_THK_VM_Get:
    ...
    def pre_thk_moving_avg(merge_df, Pre_Target, Pre_Thk_Para, Pre_Thk_Period):

        level_1q = merge_df[Pre_Thk_Para].quantile(0.25)
        level_3q = merge_df[Pre_Thk_Para].quantile(0.75)

        IQR   = level_3q - level_1q
        sigma = 3
        merge_df = merge_df[
            (merge_df[Pre_Thk_Para] <= level_3q + (IQR * sigma)) &
            (merge_df[Pre_Thk_Para] >= level_1q - (IQR * sigma))
        ].copy()
        ...
        for j, i in enumerate(merge_df['pre_eq_ch'].unique()):
            ...
            pre_thk_raw   = merge_df[merge_df['pre_eq_ch'] == i][[Pre_Thk_Para, 'pre_oper_time']].rolling(window=Pre_Thk_Period, on='pre_oper_time', min_periods=min_count).mean()[Pre_Thk_Para]
            pre_thk_count = merge_df[merge_df['pre_eq_ch'] == i][[Pre_Thk_Para, 'pre_oper_time']].rolling(window=Pre_Thk_Period, on='pre_oper_time', min_periods=min_count).count()[Pre_Thk_Para]
        ...
        Pre_Thk_Para_Mean = merge_df[Pre_Thk_Para].mean()

        merge_df['Pre_Thk']       = pre_thk_list - Pre_Thk_Para_Mean
        merge_df['Pre_Thk_Count'] = pre_thk_count_list

        return merge_df


    def pre_thk_ed_ex_moving_avg(merge_df, Pre_Target, Pre_Thk_Para, Pre_Thk_Period):

        level_1q = merge_df['BIAS'].quantile(0.25)
        level_3q = merge_df['BIAS'].quantile(0.75)

        IQR   = level_3q - level_1q
        sigma = 3
        merge_df = merge_df[
            (merge_df['BIAS'] <= level_3q + (IQR * sigma)) &
            (merge_df['BIAS'] >= level_1q - (IQR * sigma))
        ].copy()
        ...
        for j, i in enumerate(merge_df['pre_eq_ch'].unique()):
            ...
            pre_thk_raw   = merge_df[merge_df['pre_eq_ch'] == i][['BIAS', 'pre_oper_time']].rolling(window=Pre_Thk_Period, on='pre_oper_time', min_periods=min_count).mean()['BIAS']
            pre_thk_count = merge_df[merge_df['pre_eq_ch'] == i][['BIAS', 'pre_oper_time']].rolling(window=Pre_Thk_Period, on='pre_oper_time', min_periods=min_count).count()['BIAS']
